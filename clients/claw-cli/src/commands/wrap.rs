//! `clawai wrap <server_name>` — rewrite MCP client config to route through ClawAI.
...
pub fn run(server_name: &str, client_hint: &str) -> Result<()> {
    ...
    if is_wrapped(server) {
        println!("\"{}\" is already wrapped by ClawAI in {}.", server_name, client.display_name);
        return Ok(());
    }
    ...
    server_obj.insert("command".to_string(), json!("clawai"));
    server_obj.insert("args".to_string(), json!(proxy_args));
    server_obj.insert("_clawai_original".to_string(), Value::Object(original));

    write_config(&client.config_path, &config)?;

    println!("Wrapped \"{}\" in {}", server_name, client.display_name);
    println!();
    println!("ClawAI will now intercept all MCP communication with this server.");
    println!("Restart {} for changes to take effect.", client.display_name);
    println!();
    println!("To undo: clawai unwrap {}", server_name);
...
mod tests {
    ...
    fn test_wrap_modifies_config_correctly() {
        ...
        server.insert("_clawai_original".to_string(), Value::Object(original));

        // Verify.
        assert_eq!(config["mcpServers"]["my-server"]["command"], "clawai");
        ...
        // _clawai_original preserved.
        let orig = &config["mcpServers"]["my-server"]["_clawai_original"];
    ...
    fn test_wrap_preserves_other_servers() {
        ...
        server_a.insert("command".to_string(), json!("clawai"));
        server_a.insert("args".to_string(), json!(["proxy", "--", "node", "a.js"]));
        server_a.insert("_clawai_original".to_string(), json!({"command": orig_cmd, "args": orig_args}));
