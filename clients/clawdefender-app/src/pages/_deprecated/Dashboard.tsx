...
export function Dashboard() {
  ...
  const [aiStatus, setAiStatus] = useState<{ loaded: boolean; model_name: string | null; mock_mode?: boolean } | null>(null);
  ...
  const fetchDashboardData = useCallback(() => {
    ...
    invoke<McpClient[]>("detect_mcp_clients")
      .then(async (clients) => {
        ...
        for (const c of clients.filter((cl) => cl.detected)) {
          try {
            const srvs = await invoke<McpServer[]>("list_mcp_servers", { client: c.name });
            allServers.push(...srvs);
          } catch { /* client may not have servers */ }
        }
    ...
    <div className="p-6 space-y-6">
      ...
      {aiStatus?.mock_mode && (
        <div className="rounded-lg border border-amber-500/30 bg-amber-500/10 px-4 py-3 flex items-center justify-between">
          ...
          <Link
            to="/settings"
            className="text-xs font-medium text-amber-400 hover:text-amber-300 underline underline-offset-2 whitespace-nowrap ml-3"
          >
            Set up AI Model
          </Link>
        </div>
