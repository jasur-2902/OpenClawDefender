...
interface AIAnalysisProps {
  ...
  onToggleModelManager: (show: boolean) => void;
}

export function AIAnalysis({ showModelManager, onToggleModelManager }: AIAnalysisProps) {
  ...
  const isSlmActive = slmStatus?.loaded && !slmStatus.mock_mode;
  ...
  return (
    <div className="rounded-lg border border-[var(--color-border)] bg-[var(--color-bg-secondary)] p-4">
      ...
      <p className="text-xs text-[var(--color-text-secondary)] mb-4">
        Use AI for smarter security analysis
      </p>

      {/* Main toggle - SLM on/off */}
      <div className="flex items-center justify-between mb-3">
        <div>
          <p className="text-sm">Use AI for smarter analysis</p>
          <p className="text-xs text-[var(--color-text-secondary)]">
            {isSlmActive
              ? `Active: ${slmStatus?.model_name ?? activeModel?.model_name ?? "Unknown model"}`
              : "Not active"}
          </p>
        </div>
        <ToggleSwitch
          ...
          onChange={(v) => {
            ...
            } else if (v && !activeModel) {
              onToggleModelManager(true);
            }
          }}
        />
      </div>

      {/* When active, show model name + change link */}
      {isSlmActive && activeModel && (
        <div className="flex items-center gap-2 mb-3 p-2 rounded-md bg-[var(--color-bg-tertiary)]">
          <span className="inline-block w-2 h-2 rounded-full bg-[var(--color-safe)]" />
          <span className="text-xs font-medium">{activeModel.model_name}</span>
          ...
          <button
            onClick={() => onToggleModelManager(true)}
      ...
      {/* When no model, show download prompt */}
      {!isSlmActive && !activeModel && (
        <div className="p-3 rounded-md border border-dashed border-[var(--color-border)] text-center mb-3">
          <p className="text-xs text-[var(--color-text-secondary)] mb-2">
            Download a model for AI-powered analysis
          </p>
          <button
            onClick={() => onToggleModelManager(true)}
            className="px-3 py-1.5 rounded-md text-xs font-medium bg-[var(--color-accent)] text-white hover:bg-[var(--color-accent-hover)]"
          >
            Set Up AI Model
          </button>
        </div>
      ...
      {/* Cloud analysis sub-toggle */}
      <div className="pt-3 border-t border-[var(--color-border)]">
        <div className="flex items-center justify-between mb-2">
          <div>
            <p className="text-sm">Cloud analysis</p>
            <p className="text-xs text-[var(--color-text-secondary)]">
              Send event metadata to cloud AI for deeper analysis
            </p>
          </div>
          <ToggleSwitch
            ...
            onChange={(v) => {
              if (v) {
                ...
                if (cloudProviders.length === 0) loadCloudProviders();
        ...
        {isCloudActive && cloudUsage && (
          <p className="text-xs text-[var(--color-text-secondary)] mb-2">
            Monthly usage: ~${cloudUsage.estimated_cost_usd.toFixed(2)} ({cloudUsage.total_requests} analyses)
          </p>
        )}
      </div>

      {/* Model Manager (expanded) */}
      {showModelManager && (
        <div className="mt-4 pt-4 border-t border-[var(--color-border)]">
          <div className="flex items-center justify-between mb-3">
            <h3 className="text-sm font-semibold">Model Manager</h3>
            <button
              onClick={() => onToggleModelManager(false)}
          ...
          <div className="rounded-lg border border-[var(--color-border)] bg-[var(--color-bg-tertiary)]">
            ...
            >
              <div className="flex items-center gap-2">
                <span className="text-xs font-semibold text-[var(--color-text-secondary)]">Cloud API</span>
                <span className="text-[10px] px-2 py-0.5 rounded-full bg-[var(--color-warning-subtle)] text-[var(--color-warning)] font-medium">
                  Advanced
                </span>
              </div>
            ...
            {cloudExpanded && (
              <div className="px-3 pb-3 space-y-3">
                <div className="rounded-lg bg-[var(--color-warning-subtle)] border border-[var(--color-warning)]/30 p-2">
                  <p className="text-xs text-[var(--color-text-secondary)]">
                    Cloud API sends metadata to third-party servers and costs money. Local models are private, fast, and free.
                  </p>
                </div>
...
function DownloadProgressBar({
  ...
  if (st === "failed") {
    return (
      <div className="mt-3 pt-3 border-t border-[var(--color-border)]">
        <div className="flex items-center justify-between">
          <p className="text-xs text-[var(--color-danger)] flex-1">{failedMsg || "Download failed"}</p>
          <button
            onClick={onRetry}
            className="ml-3 px-2 py-1 rounded text-[10px] font-medium bg-[var(--color-accent)] text-white hover:opacity-90 shrink-0"
          >
