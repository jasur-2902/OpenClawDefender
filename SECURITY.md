...
Rookbot employs multiple layers of security:
...
The MCP guard HTTP server requires a bearer token for all requests. The token is a cryptographically random 32-byte hex string generated on first start and stored at `~/.local/share/rookbot/server-token`. Token validation uses constant-time comparison to prevent timing side-channel attacks. Empty or missing tokens are always rejected with HTTP 401.
...
- eslogger integration is read-only observation; Rookbot never injects into other processes.
...
- **Tampered updates** — signed feed manifests and signed app updates prevent supply-chain attacks on Rookbot itself.

### What Rookbot does NOT protect against
- A pre-compromised operating system (Rookbot assumes the OS kernel is trustworthy).
...
If you discover a security vulnerability in Rookbot, please report it responsibly.

**Preferred:** [GitHub private vulnerability report](https://github.com/rookbot-io/rookbot/security/advisories/new)

**Alternative:** Email [security@rookbot.dev](mailto:security@rookbot.dev)
...
- **Privilege escalation** — any way Rookbot itself can be used to gain elevated access.
- **Audit log tampering** — circumventing or falsifying audit records.
- **Configuration injection** — manipulating policy files or Rookbot configuration through crafted MCP messages.
- **Feed signature bypass** — any way to get Rookbot to accept an unsigned or incorrectly signed feed.
...
- Issues that require a pre-compromised system (Rookbot assumes the OS is not already compromised).
