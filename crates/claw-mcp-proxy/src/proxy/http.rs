...
use claw_core::ipc::protocol::UiRequest;
            ...
            match action {
                ...
                PolicyAction::Block => {
                    ...
                    if let Some(id) = request_id(&msg) {
                        make_http_block_response(&id, "Blocked by ClawAI policy")
                    } else {
                        ...
                        match bridge.send_prompt(ui_req).await {
                            Ok(resp_rx) => {
                                match tokio::time::timeout(state.prompt_timeout, resp_rx).await {
                                    Ok(Ok(claw_core::ipc::protocol::UiResponse::Decision {
                                        action: claw_core::ipc::protocol::UserDecision::AllowOnce
                                        | claw_core::ipc::protocol::UserDecision::AllowSession,
                                        ..
                                    })) => {
                                        state.metrics.inc_allowed();
                                        forward_raw(&state, &headers, &body).await
