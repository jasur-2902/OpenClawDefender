...
/// SLM risk analysis enrichment for a pending prompt.
#[derive(Debug, Clone)]
pub struct SlmEnrichment {
...
/// Cloud swarm analysis enrichment for a pending prompt.
/// SAFETY: Swarm verdict is advisory only. Never modifies policy decisions.
#[derive(Debug, Clone)]
pub struct SwarmEnrichment {
    /// Risk level: "LOW", "MEDIUM", "HIGH", or "CRITICAL".
    pub risk_level: String,
    /// Human-readable explanation of the risk assessment.
    pub explanation: String,
...
pub struct PendingPrompt {
    ...
    /// SLM risk analysis enrichment, populated asynchronously after the prompt arrives.
    /// `None` means analysis is still pending (show spinner). `Some` means results are ready.
    pub slm_enrichment: Option<SlmEnrichment>,
    /// Cloud swarm analysis enrichment, populated asynchronously after SLM escalation.
    /// SAFETY: Swarm verdict is advisory only. Never modifies policy decisions.
    pub swarm_enrichment: Option<SwarmEnrichment>,
    /// Whether this prompt was noise-filtered (no SLM analysis needed).
...
mod tests {
    ...
    #[test]
    fn test_slm_enrichment_fields() {
        let enrichment = SlmEnrichment {
