...
use clawdefender_core::ipc::protocol::UserDecision;

pub use crate::prompt::{PendingPrompt, SlmEnrichment, SwarmEnrichment};
...
pub struct LiveStats {
    ...
    /// SLM status info for the header bar.
    pub slm_status: Option<SlmStatus>,
    /// Swarm status info for the header bar.
    pub swarm_status: Option<SwarmStatus>,
}

/// Cloud swarm status for the TUI header.
#[derive(Debug, Clone)]
pub struct SwarmStatus {
    /// Whether the swarm is enabled (API key present and config enabled).
    pub enabled: bool,
}

/// SLM engine status for the TUI header.
#[derive(Debug, Clone)]
pub struct SlmStatus {
    /// Whether the SLM is enabled and loaded.
    pub enabled: bool,
...
pub struct EventRecord {
    ...
    pub method: String,
    pub summary: String,
    /// SLM risk level for this event, if analysis was performed.
...
impl TuiState {
    ...
    /// Apply SLM enrichment to a pending prompt by ID.
    pub fn apply_slm_enrichment(&mut self, prompt_id: &str, enrichment: SlmEnrichment) {
        if let Some(prompt) = self.pending_prompts.iter_mut().find(|p| p.id == prompt_id) {
            prompt.slm_enrichment = Some(enrichment);
        }
    }

    /// Apply swarm enrichment to a pending prompt by ID.
    pub fn apply_swarm_enrichment(&mut self, prompt_id: &str, enrichment: SwarmEnrichment) {
        if let Some(prompt) = self.pending_prompts.iter_mut().find(|p| p.id == prompt_id) {
            prompt.swarm_enrichment = Some(enrichment);
        }
    }
    ...
    pub fn handle_key(&mut self, code: KeyCode, modifiers: KeyModifiers) -> bool {
        ...
        match code {
            ...
            KeyCode::Char('c') if self.mode == UiMode::PromptFocused => {
                // Open chat for the selected event (placeholder — the daemon
                // would open a browser to the chat server URL).
                if let Some(prompt) = self.pending_prompts.get(self.selected_prompt) {
                    tracing::info!(prompt_id = %prompt.id, "Chat requested for event");
                }
            }
...
fn draw_ui(frame: &mut ratatui::Frame, state: &TuiState) {
    ...
    let prompt_height = if state.pending_prompts.is_empty() {
        ...
        // Each prompt takes ~5-6 lines (extra line for SLM enrichment), plus 2 for borders.
...
fn draw_prompts(frame: &mut ratatui::Frame, state: &TuiState, area: Rect) {
    ...
    for (i, prompt) in state.pending_prompts.iter().enumerate().take(3) {
        ...
        // SLM enrichment line.
        if let Some(ref enrichment) = prompt.slm_enrichment {
            let (risk_style, risk_label) = risk_level_style(&enrichment.risk_level);
            lines.push(Line::from(vec![
                Span::raw("   SLM: "),
        ...
        } else if !prompt.noise_filtered {
            // SLM analysis still pending — show spinner.
            let spinner_chars = ["|", "/", "-", "\\"];
            let tick = (prompt.received_at.elapsed().as_millis() / 200) as usize;
        ...
        // Swarm enrichment line (shown below SLM enrichment).
        if let Some(ref enrichment) = prompt.swarm_enrichment {
            let (risk_style, risk_label) = risk_level_style(&enrichment.risk_level);
            lines.push(Line::from(vec![
                Span::raw("   Swarm: "),
                Span::styled(risk_label, risk_style),
...
fn draw_event_log(frame: &mut ratatui::Frame, state: &TuiState, area: Rect) {
    ...
    let lines: Vec<Line> = state
        ...
        .map(|e| {
            ...
            if let Some(ref risk) = e.risk_level {
                let (style, label) = risk_level_style(risk);
                spans.push(Span::raw(" "));
...
fn draw_footer(frame: &mut ratatui::Frame, state: &TuiState, area: Rect) {
    let help = match state.mode {
        UiMode::PromptFocused => "q:quit  ^C:quit  Up/Dn:select  a/s/p/d:decide  c:chat  Tab:log",
...
fn risk_level_style(level: &str) -> (Style, String) {
    match level.to_uppercase().as_str() {
        "LOW" => (Style::default().fg(Color::Green), "LOW".to_string()),
...
mod tests {
    ...
    fn test_apply_slm_enrichment() {
        ...
        assert!((enrichment.confidence - 0.88).abs() < 0.01);
    }
    ...
    #[test]
    fn test_tui_state_without_slm_unchanged() {
        // Verify that the TUI works identically when SLM is not active.
        let mut state = TuiState::default();
        assert!(state.stats.slm_status.is_none());

        let (p, rx) = make_test_prompt("p1", Duration::from_secs(30));
        state.add_prompt(p);
        assert!(state.pending_prompts[0].slm_enrichment.is_none());
        assert!(!state.pending_prompts[0].noise_filtered);
    ...
    #[test]
    fn test_noise_filtered_prompt_no_spinner() {
        let mut state = TuiState::default();
        ...
        let prompt = PendingPrompt {
            ...
            policy_rule: "prompt_all".to_string(),
            policy_message: "Manual check".to_string(),
            received_at: std::time::Instant::now(),
            timeout: Duration::from_secs(30),
            response_tx: Some(tx),
            slm_enrichment: None,
        ...
        state.add_prompt(prompt);
        // noise_filtered = true and no enrichment means no "Analyzing..." spinner.
        assert!(state.pending_prompts[0].noise_filtered);
        assert!(state.pending_prompts[0].slm_enrichment.is_none());
    }

    #[test]
    fn test_event_with_risk_level() {
        let mut state = TuiState::default();
    ...
    #[test]
    fn test_slm_status_display() {
        let mut stats = LiveStats::default();
        // Disabled by default.
        assert!(stats.slm_status.is_none());

        // Set enabled.
        stats.slm_status = Some(SlmStatus {
            enabled: true,
            model_name: "tinyllama-1.1b".to_string(),
            avg_latency_ms: 45.2,
        });
        let status = stats.slm_status.as_ref().unwrap();
        assert!(status.enabled);
        assert_eq!(status.model_name, "tinyllama-1.1b");
    }
}
