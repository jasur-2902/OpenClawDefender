# Rookbot GUI — Frequently Asked Questions
...
1. **During onboarding**: Rookbot automatically detects MCP clients and their servers. Select which servers to protect and click "Protect These."
...
### Can I use Rookbot without the GUI?

Yes. Rookbot's core is a daemon that runs in the background. You can manage it entirely through the CLI:
...
- **Close window** (red X or Cmd+W): If "Minimize to tray" is enabled in Settings, Rookbot continues running in the system tray. The daemon stays active.
...
### Why does Rookbot need Full Disk Access?

Rookbot reads MCP client configuration files to detect servers and modify them for wrapping. On macOS, these files may be in locations that require Full Disk Access (e.g., Claude Desktop's config in `~/Library/Application Support/`). Without this permission, server detection may fail.

### How do I update Rookbot?

Rookbot supports auto-update. When an update is available:

1. A notification appears in the app
2. The update downloads in the background
3. Restart Rookbot to apply the update

You can also update via Homebrew: `brew upgrade --cask clawdefender`

### How do I uninstall Rookbot?

1. Quit Rookbot from the system tray
2. Run `claw unwrap --all` to restore original MCP server configurations
3. Run `claw daemon stop` to stop the daemon
4. Delete `/Applications/Rookbot.app`
5. Remove `~/.config/rookbot/` (configuration) and `~/Library/Logs/clawdefender/` (logs)
...
For high-risk prompts, Rookbot recommends denying and displays the Deny button prominently.
...
Rookbot builds behavioral profiles by observing server activity over time. New servers start in "learning" mode until enough data is collected.
...
### What ports does Rookbot use?

Rookbot communicates via Unix domain sockets (IPC), not TCP ports. The socket is located at the path shown in the daemon status (typically `~/.config/rookbot/daemon.sock`).

### Does Rookbot slow down my AI tools?

Rookbot is designed for minimal latency. The proxy adds sub-millisecond overhead to MCP calls. The behavioral analysis engine runs asynchronously and does not block tool calls.

### Can I run multiple instances of Rookbot?

No. Only one instance of the Rookbot app can run at a time. The daemon is also a singleton — attempting to start a second daemon will fail gracefully.

### Where are logs stored?

- **macOS**: `~/Library/Logs/clawdefender/`
- **Linux**: `~/.local/share/rookbot/logs/`

Set the log level in Settings (Trace, Debug, Info, Warn, Error).

### Where is the configuration stored?

Configuration files are in `~/.config/rookbot/`:
