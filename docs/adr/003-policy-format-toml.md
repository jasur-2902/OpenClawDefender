...
Rookbot needs a format for users to define security policies (allow, deny, prompt rules for MCP tool calls). The format must be human-readable, machine-parseable, and appropriate for version control.
...
**Rego (OPA).** Powerful but complex. Rego is a purpose-built policy language with its own evaluation model. For Rookbot's use case (pattern matching on tool names and arguments), Rego is overkill. It raises the barrier for users who just want to block a tool. We may offer Rego as an advanced option in the future.
